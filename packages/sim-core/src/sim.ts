...
export class Simulation {
  ...
  constructor(config: RunConfig, options: SimulationOptions = {}) {
    ...
    this.adoption = options.adoption ?? "instant";
    this.topology = cloneTopology(options.topology ?? DEFAULT_TOPOLOGY);
    this.world = world;
    ...
    this.bounds = bounds;
    this.colonies = this._initColonies(options.doctrines);
    this.foodSources = this._placeFoodSources(makeRng(config.seeds.food));
  }

  private _initColonies(doctrines: readonly Doctrine[] = []): Colony[] {
    return Array.from({ length: this.numColonies }, (_, id) => {
      ...
      const doctrine = cloneDoctrine(doctrines[id] ?? DEFAULT_DOCTRINE);
      const colony: Colony = {
        id,
        nestX,
        nestY,
  ...
  private _lay(ant: Ant, colony: Colony) {
    ...
    // War uses the returning state for a foodless emergency retreat. Preserve
    // the movement-time gland cost without manufacturing a food trail.
    if (ant.state === "returning" && !ant.hasFood) {
      ant.tank = Math.max(0, ant.tank - DEPOSIT_RATE);
      return;
    }
  ...
  /** Applies a live doctrine outside the trace command queue (for untraced modes such as War). */
  setColonyDoctrine(index: number, doctrine: Doctrine): void {
    this._applySetDoctrine(index, doctrine);
  }
